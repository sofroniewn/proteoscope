...
class AutoencoderLightningModule(LightningModule):
    ...
    def forward(self, images):
        ...
        quant, emb_loss, info = self.vae.quantize(latents)
        quant = self.vae.post_quant_conv(quant)
        reconstructed = self.vae.decoder(quant)

        logits = self.classifier(latents.reshape(latents.shape[0], -1))

        return reconstructed, emb_loss, logits

    def _calc_losses(self, images, labels, output_images, emb_loss, output_logits):

        loss = {}
        loss['emb_loss'] = emb_loss
        ...
        loss['loss'] = loss['reconstruction'] + self.vae_coeff * loss['emb_loss'] + self.classifier_coeff * loss['classification']
        return loss

    def training_step(self, batch, batch_idx, dataloader_idx=0):
        images = batch['image']
        labels = batch['label']
        output_images, emb_loss, output_logits = self(images)

        loss = self._calc_losses(images, labels, output_images, emb_loss, output_logits)        
